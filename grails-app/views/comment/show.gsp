	...
	<body>
		...
		<div id="show-comment" class="content scaffold-show" role="main">
			...
			<ol class="property-list comment">
				...
				<g:if test="${commentInstance?.relatedMsg}">
				<li class="fieldcontain">
					<span id="relatedMsg-label" class="property-label"><g:message code="comment.relatedMsg.label" default="Related Msg" /></span>
					
						<span class="property-value" aria-labelledby="relatedMsg-label"><g:link controller="message" action="show" id="${commentInstance?.relatedMsg?.id}">${commentInstance?.relatedMsg?.encodeAsHTML()}</g:link></span>
