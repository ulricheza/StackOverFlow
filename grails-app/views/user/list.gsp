...
<html>
	<head>
		...
		<title><g:message code="isima.user.list.title" /></title>
						...
						source: function( request, response ) {
							...
							$.getJSON(url, { term: extractLast(request.term) }, function(te){
								...
								for(i=0; i<te["users"].length; ++i)
								{
									...
									/*$("#userListContent tbody tr").last().append(
										"<td class=\"tag-cell\">" +
											"<a class=\"post-tag\" href=\"./user/show/"+te["users"][i]["id"]+"\">"+te["users"][i]["tagName"]+"</a><br/>" +
											"<div class=\"tag-short-description\">"+te["users"][i]["description"]+"</div>" +
										"</td>"
									);*/
			...
			<table id="userListContent" name="userListContent">
				<g:each in="${userInstanceList}" status="userInstanceId" var="userInstance">
					...
					<tr>
						...
						<td class="tag-cell">
							<img style="float:left; margin-right:5px;" width="32" height="32" src="data:image/png;base64,${userInstance.profileImage.encodeBase64().toString()}" />
							<div class="user-description-container">
